   ...
   молча, и тогда машина отправит человека не туда. */
{
  ...
  if (!r.ld) fail('на странице шоурума нет разметки места (LocalBusiness)');
    ...
    if (!missing.length) ok('шоурум: адрес и часы в разметке совпадают со страницей, заглушки в разметку не попали');
  }
  await c.close();
}
   ...
   стены житель не проходит. */
{
  ...
  if (!stage) fail('на главной нет интерактивной модели дома');
    ...
    ok(`модель дома: сценариев ${btns.length}, состояние объявляемо, следов за 14 шагов ${best}`);
  }
  await c.close();
}
