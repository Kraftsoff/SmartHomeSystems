...
for (const f of files) {
  ...
  /* Запрет индексации на содержательной странице убирает её из выдачи молча:
     сайт останется на месте, работать будет, а искать его перестанут. Он
     уместен только на страницах ошибки, и попасть сюда может из сборки
     превью, где стоит намеренно. */
  if (/<meta[^>]+name="robots"[^>]+noindex/i.test(s)) {
    fail(`${url}: страница закрыта от индексации`);
  }
   ...
   ни в сравнениях. */
{
  ...
  if (!sm) fail('нет sitemap.xml');
    ...
    /* Дата изменения обязана приходить из записи об изменениях, а не из
       момента сборки. Раньше все 139 адресов получали «сейчас»: карта
       говорила, что весь сайт меняется целиком при каждой выкладке, и
       обходчик переставал верить признаку — а он затем и нужен, чтобы
       подсказать, что перечитать.
       Одинаковая дата сама по себе не порок: сегодня правились все страницы.
       Порок — если она не совпадает с записью или записи нет вовсе. */
    const даты = new Map([...sm.matchAll(/<loc>([^<]+)<\/loc>\s*<lastmod>([^<]+)</g)]
      .map((m) => [m[1].replace(/^https?:\/\/[^/]+/, ''), m[2].slice(0, 10)]));
    const путьЗаписи = resolve('site-foundation/lastmod.json');
    if (!existsSync(путьЗаписи)) fail('нет записи об изменениях: дата в карте сайта взята из сборки');
    else {
      const запись = JSON.parse(readFileSync(путьЗаписи, 'utf8'));
      const расходятся = [...даты.entries()].filter(([u, d]) => запись[u] && запись[u].дата !== d);
      if (расходятся.length) {
        fail(`дата в карте расходится с записью об изменениях: ${расходятся.slice(0, 2).map(([u, d]) => `${u} ${d}≠${запись[u].дата}`).join(', ')}`);
      }
      const нет = [...даты.keys()].filter((u) => !запись[u]);
      if (нет.length) fail(`адреса без записи об изменениях: ${нет.slice(0, 3).join(', ')}`);
    }
    if (!missing.length && !extra.length) {
      ok(`карта сайта: ${inMap.size} адресов, ровно столько же страниц, дат ${new Set(даты).size}`);
    }
  }
}
...
/* Число в тексте о себе должно быть проверяемо по самому сайту. Главная
   утверждала «тринадцать инженерных систем», а тринадцати систем не
   перечислено нигде: направлений семь. Читатель, который решит посчитать,
   не сойдётся — и это ровно то, что стандарт проекта запрещает. */
{
  const данные = JSON.parse(readFileSync(resolve('site/lib/content.json'), 'utf8'));
  const направлений = Object.keys(данные.sections).filter((k) => /^functions\/[a-z-]+$/.test(k)).length;
  const дом = readFileSync(join(OUT, 'index.html'), 'utf8').replace(/<[^>]+>/g, ' ');
  const слова = { два: 2, три: 3, четыре: 4, пять: 5, шесть: 6, семь: 7, восемь: 8,
    девять: 9, десять: 10, одиннадцать: 11, двенадцать: 12, тринадцать: 13 };
  /* Считаем не одно слово, а все счётные утверждения о себе: ответы, темы,
     направления, кейсы. Каждое проверяется по самому сайту — иначе читатель,
     решивший посчитать, не сойдётся, и это ровно то, чего стандарт не
     допускает. Все совпадения, а не первое: число упоминается не единожды,
     и проверка первого проходила, пока подделка стояла во втором. */
  const счётные = [
    ['ответ', /(\d+|[А-Яа-я]+)\s+(?:инженерных\s+)?ответ[а-я]*/gi, данные.answers.length],
    ['тема', /(\d+|[А-Яа-я]+)\s+тем[ауы]?м?/gi, new Set(данные.answers.map((a) => a.cluster)).size],
    ['направление', /(\d+|[А-Яа-я]+)\s+направлени[йяе]/gi, направлений],
    ['сценарий', /(\d+|[А-Яа-я]+)\s+сценари[еийя]+/gi, 28],
  ];
  const сбои = [];
  let сверено = 0;
  for (const [имя, re, факт] of счётные) {
    for (const m of дом.matchAll(re)) {
      const n = /^\d+$/.test(m[1]) ? Number(m[1]) : слова[m[1].toLowerCase()];
      if (!Number.isFinite(n)) continue;
      сверено += 1;
      if (n !== факт) сбои.push(`${имя}: сказано ${n}, на сайте ${факт}`);
    }
  }
  if (сбои.length) fail(`главная называет числа, которых на сайте нет — ${[...new Set(сбои)].join('; ')}`);
  else ok(`счётные утверждения на главной сходятся с сайтом (сверено ${сверено})`);
}

/* План обязан оживать сам и на компьютере. Прежняя проверка смотрела только
   телефон, а условие запуска было «там, где нельзя навести курсор»: на
   десктопе план ждал мыши над собой, о чём нигде не сказано, и тринадцать
   секунд оставался неподвижным тёмным прямоугольником. Главный элемент
   страницы читался как недоделанный — и читался так у клиента. */
{
  ...
  await pg.evaluate(() => {
    ...
    new MutationObserver((rs) => {
      for (const r of rs) for (const n of r.addedNodes) {
        if (n.nodeType === 1 && n.classList.contains('footprint')) window.__walk += 1;
      }
    }).observe(st, { childList: true });
  });
  /* Окно ожидания то же, что у телефонной проверки, и по той же причине.
     В изоляции первый след появляется через 620 мс, но под полным прогоном
     рядом работают ещё десяток контекстов, и на скачивание, разбор и
     гидратацию уходит на порядок больше. Проверяемое свойство — «ходит сам»,
     без обещания срока: прогулка, которая не начинается никогда, эту проверку
     всё равно роняет. */
  let следов = 0;
  for (let i = 0; i < 60 && !следов; i += 1) {
    await pg.waitForTimeout(400);
    следов = await pg.evaluate(() => window.__walk || 0);
  }
  if (!следов) {
    /* Диагностика на месте: без неё «следов нет» одинаково выглядит и когда
       план сломан, и когда машина не успела. */
    const почему = await pg.evaluate(() => ({
      сцена: !!document.querySelector('.house-stage'),
      гидратация: !!document.querySelector('.house-stage [data-room], .house-stage svg'),
      покой: matchMedia('(prefers-reduced-motion: reduce)').matches,
      вОкне: (() => {
        const b = document.querySelector('.house-stage')?.getBoundingClientRect();
        return b ? b.bottom > 0 && b.top < innerHeight : null;
      })(),
    }));
    fail(`на компьютере план стоит мёртвым: без движения мыши ни одного следа (${JSON.stringify(почему)})`);
  } else ok(`план на компьютере: оживает сам, следов ${следов}`);
  await c.close();
}

/* Таблица, которую заменила интерактивная подача, не должна остаться на
   странице рядом с заменой. Так уже вышло дважды: смета показывалась и
   карточками, и диаграммой, типы объектов — и таблицей, и выбором. Читатель
   видит одно и то же дважды и решает, что сайт собран небрежно. */
{
  /* Признак — заголовок вырезанного раздела в его прототипной оправе
     (class="cluster-h"), а не текст: текст законно переезжает внутрь замены,
     и по нему проверка ловила саму замену. Шаблон рисует свой заголовок без
     этого класса, поэтому одно от другого отличимо. */
  const пары = [
    ['/pricing/', 'est-seg', 'cluster-h">Из чего складывается смета'],
    ['/solutions/', 'obj-tab', 'cluster-h">Что меняется от типа объекта'],
  ];
  for (const [url, признак, остаток] of пары) {
    const f = files.find((x) => `/${relative(OUT, x).replace(/index\.html$/, '')}` === url);
    if (!f) continue;
    const h = readFileSync(f, 'utf8');
    if (!h.includes(признак)) fail(`${url}: интерактивная подача не отрисовалась`);
    else if (h.includes(остаток)) fail(`${url}: рядом с интерактивной подачей осталась таблица, которую она заменила`);
  }
  ok('заменённые таблицы со страниц убраны, дублей нет');
}
