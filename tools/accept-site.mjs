...
for (const f of files) {
  ...
  if (!t) fail(`${url}: нет title`);
    ...
    /* Граница поднята с 60 до 70 и приставка бренда убрана. При старом
       правиле имя компании съедало двенадцать знаков в каждом заголовке, и
       семьдесят два обрывались многоточием посреди вопроса — «Чем инженерный
       умный дом отличается от… — MiMiSmart». Заголовок это главная цель
       нажатия; обрывок в ней хуже, чем отсутствие имени, которое всё равно
       объявлено разметкой организации на каждой странице. */
    if (t.length > 70) fail(`${url}: title ${t.length} знаков вместо 70 и меньше`);
