   ...
   не сойдётся — и это ровно то, что стандарт проекта запрещает. */
{
  ...
  /* Все совпадения, а не первое: число упоминается на главной не один раз, и
     проверка первого проходила, пока подделка стояла во втором. */
  const все = [...дом.matchAll(/(\d+|[А-Яа-я]+)\s+направлени[йяе]/gi)]
    .map((m) => (/^\d+$/.test(m[1]) ? Number(m[1]) : слова[m[1].toLowerCase()]))
    .filter((n) => Number.isFinite(n));
  const плохие = все.filter((n) => n !== направлений);
  if (плохие.length) fail(`главная обещает ${[...new Set(плохие)].join(' и ')} направлений, на сайте их ${направлений}`);
  else if (все.length) ok(`число направлений на главной сходится с сайтом: ${направлений} (упомянуто ${все.length} раз)`);
}
