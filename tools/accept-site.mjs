   ...
   и пройденное в одной ничего не говорит о другой. */
{
  ...
  /* Сообщение об успехе печатается только при успехе. Раньше оно стояло за
     циклом безусловно и рапортовало «обе темы держат AA» одновременно с
     записью нарушения — я сам на это купился, читая вывод. */
  if (clean === 2) console.log('контраст: обе темы держат AA');
}
