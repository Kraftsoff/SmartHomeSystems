...
/* Число о себе обязано сверяться с самим прототипом. «13 инженерных систем»
   пережило правку заголовка и описания в разметке для пересылки: её глазами
   не читают, и непроверяемое число уехало бы в каждую ссылку, которой
   поделились. Направлений семь — столько, сколько разделов есть. */
{
  const исходник = readFileSync(resolve(FILE), 'utf8');
  const мета = [...исходник.matchAll(/content="([^"]*)"/g)].map((m) => m[1]).join(' ');
  const слова = { семь: 7, восемь: 8, девять: 9, десять: 10, одиннадцать: 11,
    двенадцать: 12, тринадцать: 13 };
  for (const m of мета.matchAll(/(\d+|[А-Яа-я]{4,12})\s+инженерн[а-я]+\s+систем/gi)) {
    const n = /^\d+$/.test(m[1]) ? Number(m[1]) : слова[m[1].toLowerCase()];
    if (Number.isFinite(n)) problems.push(`в мета-разметке «${m[0]}» — число, которого на сайте не перечислено`);
  }
}
