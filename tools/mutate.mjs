...
const MUTATIONS = [
  { name: 'вопрос «сколько» остался без числа',
    /* Ответ без числа и без названной недостающей цифры проигрывает молча:
       машина процитирует того, кто цифру дал. */
    file: 'site/lib/content.json', all: true,
    from: '⚠️ поставить наши сроки по стадиям', to: 'поставить наши сроки по стадиям',
    expect: 'без числа и без пометки' },
  ...
  { name: 'оговорка раздела не выведена', file: 'site/app/[...path]/page.tsx', rebuild: true,
    /* Укорачиваем текст, а не убираем блок: `false &&` ломает сужение типа,
       сборка падает, и приёмка уходит проверять прежний site/out. */
    from: "{`⚠️ ${rec.prov.replace(/^⚠️\\s*/, '')}`}", to: '{`⚠️ ${rec.prov.slice(0, 4)}`}',
    expect: 'оговорка раздела не выведена' },
  { name: 'сравнение без таблицы сравнения', file: 'site/app/[...path]/page.tsx', rebuild: true,
    from: '{cmp && cmp.rows.length > 1 && (', to: '{cmp && cmp.rows.length > 99 && (',
    expect: 'сравнивать нечего' },
