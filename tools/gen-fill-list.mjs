...
/* Сценарии появились позже обхода и в него не попали: две пометки на сайте
   не находили себя в списке. Новый раздел данных надо вносить сюда же,
   иначе он молча выпадает из того, что ждёт ответа клиента. */
walk(data.scenarios, 'сценарии');
