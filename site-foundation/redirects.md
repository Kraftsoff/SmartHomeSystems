...
Судьба этого слоя зависит от нерешённого вопроса «оставляем ли e-commerce в V1»
(`../tz-site/99`, п. 9). Пока решения нет — **не редиректить**: если корзина/аккаунт остаются,
маршруты живут; если нет — отдавать 410 (Gone) вместо 301, чтобы не плодить бессмысленные
перенаправления. `/ai` — назначение неизвестно, перед решением открыть и посмотреть.
