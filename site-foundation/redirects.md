...
## ⛔ Перед любой из этих строк: идентификация доменов, срок 1 сентября 2026

С 1 сентября по 569-ФЗ изменить NS-серверы домена `.ru` или `.рф` можно только после
идентификации администратора через Госуслуги. Пока она не пройдена, **ни одно правило из этого
файла применить нельзя**: чтобы редиректы заработали, домены должны смотреть на новый хостинг,
а смена NS — как раз запрещённая операция.

Подробности и что именно проверить — [[16 — Что делать дальше (сводка для решения)]].
...
# Часть C. Сведение доменов — заготовка под решение клиента

Замер 27.08 показал, что доменов у бренда пять и они конкурируют между собой: `mmsmart.ru` и
`mimismart.com` отдают один и тот же заголовок «Купить умный дом в Москве — Заказать систему
умный дом по ценам от производителя — MiMiSmart». Для поиска это две страницы, претендующие на
один запрос; для ИИ — одна сущность с противоречащими описаниями.

Отдельно и серьёзнее: на одном из доменов опубликован текст, отрицающий связь с российским
производством. Подробности и что нужно выяснить — в `../audit/08-serp-baseline.md`.

## Что решает клиент

| Вопрос | Почему без ответа нельзя двигаться |
|---|---|
| Какой домен становится единственным | от этого зависит, куда ведут все 194 правила и какой адрес идёт в `canonical` |
| Кому принадлежат остальные четыре | редирект можно поставить только на своём домене |
| Что делать с доменом, отрицающим связь с РФ | если он не наш — это вопрос к юристам, а не к редиректам; если наш — текст снимается до запуска |

## Что делается сразу после ответа

1. **301 со всех четырёх на выбранный**, постранично по карте части B, а не на главную:
   редирект всего домена на главную читается поиском как soft-404, и вес не передаётся.
2. **`canonical` на выбранный домен** на каждой странице — уже реализовано, подставляется
   константа `SITE` в `site/lib/content.ts`. <span>⚠️ заполнить: боевой домен</span>
3. **`sameAs` в разметке организации** — перечисление подтверждённых ресурсов бренда
   (карточки в справочниках, сообщества, магазины приложений). Это то, чем сущность
   объявляется машине: без списка ИИ достраивает его сам и притягивает чужое.
   <span>⚠️ заполнить: список подтверждённых ресурсов бренда</span>
   Из замера уже известны два адреса приложений — App Store `id6444528782` (издатель
   «SMARTBRIK, OOO») и Google Play `com.mimismart.app`. <span>⚠️ подтвердить: наши ли это
   приложения и совпадает ли издатель с юрлицом сайта</span>
4. **Проверка через месяц после переезда**: тот же замер по 15 покупательским запросам
   (`../audit/09-serp-protocol-rf.md`), чтобы увидеть, склеились ли сущности.

Пока пункт 1 не решён, запускать сайт можно — но брендовый запрос будет работать против него,
и это не лечится содержанием сайта.
