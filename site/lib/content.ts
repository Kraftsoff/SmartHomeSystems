...
export function pageTitle(t: string) {
  /* Заголовок — это сам вопрос, без приставки бренда. Приставка стоила
     двенадцать знаков в каждом заголовке, и семьдесят два из ста тридцати
     девяти обрывались многоточием посреди вопроса: «Чем инженерный умный дом
     отличается от… — MiMiSmart». Заголовок — главная цель нажатия, обрывок
     в этой роли хуже, чем отсутствие имени компании, которое всё равно
     объявлено разметкой организации на каждой странице.
     Приставка добавляется, только если после неё остаётся запас. */
  const full = t + SUFFIX;
  if (full.length <= 60) return full;
  if (t.length <= 70) return t;
  /* Длиннее семидесяти — режем по границе смысла, а не по числу знаков,
     и сохраняем вопросительный знак: заголовок обязан остаться вопросом. */
  const зона = t.slice(0, 68);
  const граница = Math.max(зона.lastIndexOf(' — '), зона.lastIndexOf(', '), зона.lastIndexOf(' ('));
  const тело = граница > 34 ? зона.slice(0, граница) : зона.slice(0, зона.lastIndexOf(' '));
  /* Служебное слово на конце оставляет вопрос висеть: «…DIY-решений с?».
     Отбрасываем предлоги и союзы, оказавшиеся последними после реза. */
  const хвост = /\s(?:с|со|в|во|к|ко|о|об|обо|у|за|на|по|из|от|до|при|для|над|под|про|без|через|и|а|но|или|же|ли|бы)$/i;
  let чисто = тело.replace(/[\s,(—-]+$/, '');
  while (хвост.test(чисто)) чисто = чисто.replace(хвост, '');
  return `${чисто}?`;
}
